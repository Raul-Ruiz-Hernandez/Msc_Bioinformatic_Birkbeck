...
# Select top N markers from each orig.ident group to form the variable 
top_n_markers_per_group <- 500 # You can adjust this number
# -- Section Entropy---
...
# population_names <- c("Transitional", "Naïve", "IgM Memory", "Classical Memory", "Double Negative")
# cd19_status <- c("+", "+", "+", "+", "+")
# igd_status <- c("+", "+", "+", "-", "-")
# cd27_status <- c("-", "-", "+", "+", "-")
...
classical_file_path <- paste0(data_dir, "classical_filtered_feature_bc_matrix.h5")
classical.data <- Read10X_h5(filename = classical_file_path)
cat("Loaded classical.data with dimensions:", dim(classical.data), "\n\n")
...
# Load igmmem data
cat("Loading igmmem_filtered_feature_bc_matrix.h5 ...\n")
igmmem_file_path <- paste0(data_dir, "igmmem_filtered_feature_bc_matrix.h5")
igmmem.data <- Read10X_h5(filename = igmmem_file_path)
cat("Loaded igmmem.data with dimensions:", dim(igmmem.data), "\n\n")
...
trans_file_path <- paste0(data_dir, "trans_filtered_feature_bc_matrix.h5")
trans.data <- Read10X_h5(filename = trans_file_path)
cat("Loaded trans.data with dimensions:", dim(trans.data), "\n\n")
...
sample_names <- c("classical", "dn", "igmmem", "naive", "trans")
...
# Create object for classical
classical_seurat <- CreateSeuratObject(counts = classical.data, project = sample_names[1])
cat("Created Seurat object for", sample_names[1], "with", ncol(classical_seurat), "cells.\n")
...
# Create object for igmmem
igmmem_seurat <- CreateSeuratObject(counts = igmmem.data, project = sample_names[3])
cat("Created Seurat object for", sample_names[3], "with", ncol(igmmem_seurat), "cells.\n")
...
# Create object for trans
trans_seurat <- CreateSeuratObject(counts = trans.data, project = sample_names[5])
cat("Created Seurat object for", sample_names[5], "with", ncol(trans_seurat), "cells.\n")

# You now have 5 individual Seurat objects:
# classical_seurat, dn_seurat, igmmem_seurat, naive_seurat, trans_seurat
...
HB6 <- merge(
  x = classical_seurat,     # Start with the first object
  y = list(dn_seurat, igmmem_seurat, naive_seurat, trans_seurat), # A list of the remaining objects
...
count_layer_names <- c("counts.classical", "counts.dn", "counts.igmmem", "counts.naive", "counts.trans")
...
# If your "Sample" column represents the same groupings as orig.ident, you can use it for grouping plots
...
VlnPlot(HB6.a_qc1, features = "nCount_RNA", group.by = "orig.ident", ncol = 1) + ggtitle("nCount_RNA distribution per Sorted Population")

# Visualize the distribution of nCount_RNA for each original sorted population
VlnPlot(HB6.a_qc1, features = "nFeature_RNA", group.by = "orig.ident", ncol = 1) + ggtitle("nFeature_RNA distribution per Sorted Population")

# Your original plot command
vln_plot_percent_mt <- VlnPlot(
  HB6.a_qc1,
  features = "percent.mt",
  group.by = "orig.ident", # Ensure this column exists and is what you want to group by
  ncol = 1,
  pt.size = 0.1 # Optional: adjust point size if you show individual points
) +
ggtitle("percent.mt on Sorted Population") +
...
VlnPlot(HB6.a_qc1, features = "percent.ribo", group.by = "orig.ident", ncol = 1) + ggtitle("percent.ribo on Sorted Population")
...
grouping_variable <- "orig.ident" # Group by original sample identity as per template

# --- Check if the grouping variable exists ---
if (!grouping_variable %in% colnames(HB6.a_qc1@meta.data)) {
    stop(paste0("Error: Grouping variable '", grouping_variable, "' (orig.ident) not found in object metadata."))
}
...
# --- Generate and print a VlnPlot for each numeric metadata feature, grouped by orig.ident ---
...
if (length(numeric_metadata_features) > 0) {
    for (feature in numeric_metadata_features) {
        ...
        # Generate the VlnPlot for the current feature, grouped by orig.ident
        # ncol=1 ensures each plot is in a separate row for easier viewing
        vln_plot <- VlnPlot(
            object = HB6.a_qc1,
            features = feature,         # Plot only the current feature
            group.by = grouping_variable, # Group by orig.ident
...
# cell_orig_ident <- HB6.a_qc$orig.ident # Get original sample identities
...
# Optional: Get cell identities if you want to color the points by cluster or other grouping
# cell_identities <- Idents(HB6.a_qc1) # Get current identities (e.g., clusters)
# cell_orig_ident <- HB6.a_qc1$orig.ident # Get original sample identities
...
ggplot_ribo_mt_scatter <- ggplot(plot_data_df, aes(x = percent.ribo, y = percent.mt)) +
    ...
    ggtitle("percent.ribo vs percent.mt ")
...
# 
# # Visualize the distribution of percent.ribo for each original sorted population
# VlnPlot(HB6.a_qc1, features = "percent.ribo", group.by = "orig.ident", ncol = 1) + ggtitle("percent.ribo distribution per Sorted Population")
```
...
## IMPROVE CLUSTER MATCHING WITH ORIG.IDENT

This strategy aims to use genes that best distinguish your known orig.ident populations.

```{r}
# --- SCRIPT TO IMPROVE CLUSTER MATCHING WITH ORIG.IDENT ---

# --- Step 2: "Supervised" Variable Feature Selection based on orig.ident DEGs ---
cat("Finding markers for each orig.ident population to guide variable feature selection...\n")
# Ensure Idents are set to orig.ident for this FindAllMarkers call, or use group.by
Idents(HB6.a_qc1) <- HB6.a_qc1$orig.ident

orig_ident_DEGs <- FindAllMarkers(
  HB6.a_qc1,
  assay = "RNA", # Assuming your normalized data is in the RNA assay
  slot = "data", # Use normalized data to find markers
  only.pos = TRUE, # Focus on positive markers for each group
  min.pct = 0.20, # Adjust as needed: detected in at least 20% of cells in the group
  logfc.threshold = 0.25, # Adjust as needed
  verbose = TRUE # Set to TRUE for more output
)

cat("Found", nrow(orig_ident_DEGs), "total positive markers across orig.ident groups.\n")

# Select top N markers from each orig.ident group to form the variable feature list.
# This helps ensure representation from all your sorted populations.
# Note: 'top_n_markers_per_group' is assumed to be defined elsewhere in your script.
selected_variable_features <- orig_ident_DEGs %>%
  dplyr::group_by(cluster) %>% # 'cluster' column in FindAllMarkers output refers to orig.ident groups here
  dplyr::slice_max(n = top_n_markers_per_group, order_by = avg_log2FC, with_ties = FALSE) %>%
  dplyr::ungroup() %>% # Ungroup before pulling genes
  dplyr::pull(gene) %>%
  unique()

cat("Selected", length(selected_variable_features), "unique top markers from orig.ident groups to use as Variable Features.\n")

# Set these selected genes as the VariableFeatures for HB6.a_qc1
VariableFeatures(HB6.a_qc1) <- selected_variable_features
```

Which genes?

```{r}
View(as.data.frame(selected_variable_features))
```

save those genes:

```{r}
write.csv(data.frame(SelectedGenes = selected_variable_features), "selected_variable_genes_list.csv", row.names = FALSE)
# or 
# writeLines(selected_variable_features, "selected_variable_genes_list.txt")
```
...
# --- Step 3: Scale Data (using the new Variable Features and regressing variables) ---
cat("Scaling data using the selected Variable Features and regressing out specified variables...\n")
# This will scale ONLY the genes now in VariableFeatures(HB6.a_qc1)
HB6.a_qc1 <- ScaleData(HB6.a_qc1, 
...
# Color the PCA plot by the 'orig.ident' metadata column
DimPlot(HB6.a_qc1, reduction = 'pca', group.by = 'orig.ident')

# If you want to keep the legend to see which color corresponds to which orig.ident,
...
Seurat applies a graph-based clustering approach, the approach to partitioning the cellular distance matrix into clusters is heavily inspired by recent manuscripts which applied graph-based clustering approaches to scRNA-seq data [[SNN-Cliq, Xu and Su, Bioinformatics, 2015]](#0) and CyTOF data [[PhenoGraph, Levine *et al*., Cell, 2015]](#0). Briefly, these methods embed cells in a graph structure - for example a K-nearest neighbor (KNN) graph, with edges drawn between cells with similar feature expression patterns, and then attempt to partition this graph into highly interconnected 'quasi-cliques' or 'communities'.

As in PhenoGraph, we first construct a KNN graph based on the euclidean distance in PCA space, and refine the edge weights between any two cells based on the shared overlap in their local neighborhoods (Jaccard similarity). This step is performed using the `FindNeighbors()` function, and takes as input the previously defined dimensionality of the dataset (first 14 PCs).
...
*Uniform Manifold Approximation and Projection (UMAP) was performed based on the first 14 PCs, using the implementation in the python umap-learn package with correlation as the distance measure in the PC space. UMAP projections were produced on both two-dimensional and three dimensional spaces.*
...
# Rename cluster 5 to "Trans"
# HB6.a_qc1 <- RenameIdents(HB6.a_qc1, `5` = "Trans", `2` = "Naive", )
...
DimPlot(object = HB6.a_qc1, reduction = "umap", group.by = "orig.ident", label = TRUE, repel = TRUE) + NoLegend()
...
DimPlot(object = HB6.a_qc1, reduction = "umap", group.by = "orig.ident", split.by = "seurat_clusters")
```
...
cluster_origin_counts <- table(Idents(HB6.a_qc1), HB6.a_qc1$orig.ident)
...
parameter_subtitle <- paste0(
  ...
  "top_n_markers_per_group: ", top_n_markers_per_group, "\n", 
...
  - Lower values (closer to 0) indicate clusters are, on average, less mixed with respect to orig.ident.

```{r}
# citation("entropy")
```

**AMI Score:** Agreement between clustering and orig.ident labels (range typically 0 to 1, higher is more agreement).

  - Lower values (closer to 0) suggest the clustering is more independent of orig.ident (often desired after batch correction).

  - Higher values (closer to 1) suggest the clustering strongly reflects orig.ident (could be biological difference or batch effect).
...
original_samples <- HB6.a_qc1$orig.ident
...
cat("\nWeighted Average Entropy (orig.ident per cluster):\n")
...
cat("\nAdjusted Mutual Information (AMI) between clusters and orig.ident:\n")
print(ami_score)
```
...
original_samples_subset <- HB6.a_qc1_large_clusters$orig.ident
...
cat("\nWeighted Average Entropy (orig.ident per cluster - excluding small clusters):\n")
...
cat("\nAdjusted Mutual Information (AMI) between remaining clusters and orig.ident:\n")
...
**AMI Score (filtered):** Agreement between clustering and orig.ident labels for the remaining cells/clusters.
...
RotatedAxis() +
ggtitle(paste("Key Canonical Marker Expression by", grouping_variable_for_dge)) +
theme(
...
# --- 1. Create Your Data Frame ---
# It's best to copy your data into a text format that R can read easily.
# Here, I'm using `read.table` to parse the text you provided.
score_data_text <- "
WAE_score	AMI_score	IG_TCR
0.5803	0.4829	All_excluded
0.5698	0.4774	Keep_all_IG_TCR
0.5544	0.4901	Only_IgVDJ_removed
0.5893	0.4669	No_Ig_Const
0.5630	0.4792	No_IgLL
0.5751	0.4754	No_TCR
0.5412	0.5061	Keep_IgConstant
0.4476	0.5596	top_2000
0.4360	0.5445	top_1000
0.4357	0.5674	top_750
0.4399	0.5650	top_500
0.4399	0.5635	top_350
0.4418	0.5633	top_200
0.4651	0.5511	top_100
0.5382	0.5097	top_60
0.6829	0.4331	top_30
"

# Read the text data into a data frame
# Note: I replaced spaces in your 'IG/TCR' column with underscores for easier parsing.
score_df <- read.table(text = score_data_text, header = TRUE, sep = "\t")

# --- 2. Add a Category Column for Better Visualization ---
# This helps distinguish between the two types of experiments you ran.
score_df <- score_df %>%
  mutate(Analysis_Type = case_when(
    grepl("top_", IG_TCR) ~ "Selecting Top N Genes (positive control)",
    TRUE                 ~ "g/TCR selective Filtering"
  ))

# --- 3. Create the Scatter Plot ---
score_plot <- ggplot(score_df, aes(x = WAE_score, y = AMI_score)) +
  # Add points, colored by the analysis type
  geom_point(aes(color = Analysis_Type), size = 4, alpha = 0.8) +
  
  # Add non-overlapping labels for each point
  geom_text_repel(aes(label = IG_TCR), box.padding = 0.5, max.overlaps = Inf) +
  
  # Add titles and labels
  labs(
    title = "Comparison of Clustering Performance",
    subtitle = "Optimal results are in the top-left corner (Low WAE, High AMI)",
    x = "WAE score (Lower is Better)",
    y = "AMI score (Higher is Better)",
    color = "Analysis Type"
  ) +
